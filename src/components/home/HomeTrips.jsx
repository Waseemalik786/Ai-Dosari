...
export default function HomeTrips() {
  ...
  const whyTitle = t('whyTitle');
  const whyList = t('whyList', { returnObjects: true });
  const safetyTitle = t('safetyTitle');
  const safetyDesc = t('safetyDesc');
  const stats = t('stats', { returnObjects: true });
  const imageAlt = t('imageAlt');

  return (
    <section id="trips" className="py-7 bg-gradient-to-b from-gray-50 to-white" dir={i18n.language === "ar" ? "rtl" : "ltr"}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">{sectionDesc}</p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
          ...
          <div>
            <div className="mb-8">
              <h3 className="text-2xl font-bold text-[#00627B] mb-6">{whyTitle}</h3>
            ...
            <div className="bg-[black]/1 border border-[#5F0020]/20 rounded-xl p-6">
              <div className="flex items-start">
                ...
                <div>
                  <h4 className="font-bold text-[#00627B] text-lg mb-2">{safetyTitle}</h4>
                  <p className="text-[#121212]">{safetyDesc}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {stats.map((stat, index) => (
                  <div key={index} className="text-center">
                    <div className="text-3xl font-bold text-[#00627B]">{stat.value}</div>
